...
module.exports = function() {
  ...
  if (this.params.database.type === 'YottaDB') {
    if (this.params.database.host) {
      open_params = {
        type: 'YottaDB',
        host: this.params.database.host,
        tcp_port: this.params.database.tcp_port
      };
    }
    else {
      if (!this.params.env) {
      ...
      //var ydb_path;
      var ydb_path = this.params.database.path;
      var ydb_dir = process.env.ydb_dir || this.params.database.env.ydb_dir;
      if (!ydb_dir && home) {
      ...
      var ydb_rel = process.env.ydb_rel || this.params.database.env.ydb_dir;
      ...
      var ydb_gbldir = process.env.ydb_gbldir || this.params.database.env.ydb_gbldir;
      ...
      var ydb_routines = process.env.ydb_routines || this.params.database.env.ydb_routines;
      ...
      var ydb_cli = this.params.database.env.ydb_ci;
      if (!ydb_cli && usrlib_dir) {
  ...
  if (this.params.database.type === 'BDB') {

    open_params = {
      type: this.params.database.type,
      db_library: this.params.database.db_library || '/usr/local/BerkeleyDB.18.1/lib/libdb.so',
      db_file: this.params.database.db_file || '/opt/bdb/my_bdb_database.db',
      env_dir: this.params.database.env_dir || '/opt/bdb',
      key_type: this.params.database.key_type || 'm'
    };
    ...
    //console.log('dbx-dbd open params: ' + JSON.stringify(open_params, null, 2));
    status = this.dbx.open(open_params);
  }

  if (this.params.database.type === 'LMDB') {

    open_params = {
      type: this.params.database.type,
      db_library: this.params.database.db_library || 'liblmdb.so',
      env_dir: this.params.database.env_dir || '/opt/lmdb',
      key_type: this.params.database.key_type || 'm'
    };
